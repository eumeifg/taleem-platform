        ...
        function(
            ...
            return FormView.extend({
                ...
                events: {
                    ...
                    'change input[name="org_type"]': 'organizationTypeChange',
                    'change select[name="organization"]': 'organizationChange',
                    'change select[name="college"]': 'collegeChange'
                },
                liveValidationFields: [
                    ...
                    'honor_code',
                    'terms_of_service',
                    // 'phone_number', Commenting Code due to the: TA3-1858
                ],
                formType: 'register',
                ...
                // schoolField: '.select-school',
                // schoolSelect: '#register-school',
                // organizationField: '.select-organization',
                // organizationSelect: '#register-organization',
                // departmentField: '.select-department',
                // collegeField: '.select-college',
                // departmentSelect: '#register-department',
                // collegeSelect: '#register-college',
            ...
            // These are reset to true on form submission.
                ...
                renderFields: function(fields, className, ID) {
                    var html = [],
                        i,
                ...
                buildForm: function(data) {
                    ...
                    for (i = 0; i < len; i++) {
                        ...
                        // Commenting Code due to the: TA3-1858
                        // if (field.required || academicInfoFields.has(field.name) || field.name === 'phone_number') {
                        //     if(academicInfoFields.has(field.name)) {
                        //         academicInfoTab.push(field);
                        //     } else {
                        //         basicInfoTab.push(field);
                        //     }
                        // }
                    ...
                    html = this.renderFields(basicInfoTab, 'tab-pane', 'basicInfo');
                    // Commenting Code due to the: TA3-1858
                    // html.push.apply(html, this.renderFields(academicInfoTab, 'tab-pane', 'academicInfo'));
                    // html.push(HtmlUtils.HTML('<div class="tab-pane" id="optional">'));
                ...
                render: function(html) {
                    ...
                    this.postRender();
                    const phoneNumberField = document.querySelector('#register-phone_number')
                    // Commenting Code due to the: TA3-1858
                    // this.phone_number = window.intlTelInput(phoneNumberField, {
                    //     utilsScript: '/static/taleem-theme/assets/vendor/intlTelInput/utils.js',
                ...
                postRender: function() {
                    ...
                    // Commenting Code due to the: TA3-1858
                    // $(this.schoolSelect).select2({width: "100%"});
                    // $(this.organizationField).addClass('hidden');
                    // $(this.collegeField).addClass('hidden');
                    // $(this.departmentField).addClass('hidden');
                    ...
                    if (typeof registerFormInitialized == 'function') {
                        registerFormInitialized();
                    }
                },
              ...
              // Commenting Code due to the: TA3-1858
                ...
                // organizationTypeChange: function(event) {
                //     var that = this;
                //     var $el = $(event.currentTarget)[0];
                //     var organizationType = $el.value;
                //
                //     if (organizationType === 'university') {
                //         $(that.schoolField).addClass('hidden');
                //         $(that.organizationField).removeClass('hidden');
                //         $(that.organizationSelect).select2({width: "100%"});
                //     } else {
                //         $(that.organizationField).addClass('hidden');
                //         $(that.collegeField).addClass('hidden');
                //         $(that.departmentField).addClass('hidden');
                //         $(that.schoolField).removeClass('hidden');
                //         $(that.schoolSelect).select2({width: "100%"});
                //     }
                // },
                //
                // organizationChange: function(event) {
                //     var that = this;
                //     var $el = $(event.currentTarget)[0];
                //     var organizationID = $el.value;
                //     that.getColleges(organizationID);
                // },
                //
                // getColleges: function(organizationID) {
                //     var that = this;
                //     $.ajax('/taleem-organization/colleges/' + organizationID + '/?page=registration').done(
                //       function(data) {
                //         var colleges = data.colleges;
                //         $(that.collegeSelect).empty();
                //         colleges.forEach(function(college) {
                //             var option = document.createElement('option');
                //             option.text = college.text;
                //             option.value = college.id;
                //             $(that.collegeSelect)[0].appendChild(option);
                //         });
                //         $(that.collegeField).removeClass('hidden');
                //     });
                // },
                //
                ...
                //     var organizationID = $(this.organizationSelect)[0].value;
                //     if (collegeID !== '') {
                //         this.getDepartments(organizationID, collegeID);
                //     }
                // },
                //
                // getDepartments: function(organizationID, collegeID) {
                //     var that = this;
                //     $.ajax('/taleem-organization/departments/'
                //       + organizationID + '/' + collegeID + '/?page=registration').done(
                //       function(data) {
                //           var departments = data.departments;
                //           $(that.departmentSelect).empty();
                //           departments.forEach(function(department) {
                //               var option = document.createElement('option');
                //               option.text = department.text;
                //               option.value = department.id;
                //               $(that.departmentSelect)[0].appendChild(option);
                //           });
                //           $(that.departmentField).removeClass('hidden');
                //       });
                // },
                ...
                validateConfirmPasswordField: function($el) {
                    var confirmPasswordValue = $(this.confirmPassword).val();
                    var passwordFieldValue = $(this.passwordField).val();
                    var decisions = {
                    ...
                    if (passwordFieldValue !== ''){
                      if (confirmPasswordValue === ''){
                        decisions.validation_decisions['confirm_password'] = gettext(
                          'Please add password again for verification');
                      } else if (passwordFieldValue !== confirmPasswordValue) {
                        decisions.validation_decisions['confirm_password'] = gettext(
                          'Password did not match');
                      }
                      this.renderLiveValidations($el, decisions);
                    }
                },
                ...
                liveValidateHandler: function(event) {
                    ...
                    if (this.inLiveValidationFields($el)) {
                        ...
                        var fieldName = $el.attr('name') || false;
                        if (fieldName && fieldName === 'confirm_password'){
                          this.validateConfirmPasswordField($el);
                        } else {
                ...
                liveValidate: function($el) {
                    ...
                    for (i = 0; i < this.liveValidationFields.length; ++i) {
                        ...
                        //   data[field] = this.phone_number.getNumber();
                        // } else {
                        //   data[field] = $('#register-' + field).val();
                        // }
                    }
