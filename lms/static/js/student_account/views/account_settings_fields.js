(function(define, undefined) {
    ...
    define([
        ...
        var AccountSettingsFieldViews = {
            ...
            TeacherAccountFieldView: FieldViews.LinkFieldView.extend({
                fieldType: 'button',
                fieldTemplate: field_link_account_template,
                events: {
                    'click button': 'linkClicked'
                ...
                initialize: function(options) {
                    this.options = _.extend({}, options);
                    this._super(options);
                    _.bindAll(this, 'applyForTeacherAccount');
                },
                linkClicked: function(event) {
                    event.preventDefault();
                    this.toggleDisableButton(true);
                    this.applyForTeacherAccount(event);
                },
                applyForTeacherAccount: function() {
                    var view = this;
                    $.ajax({
                        type: 'GET',
                        url: view.options.linkHref,
                ...
                successMessage: function() {
                    return HtmlUtils.joinHtml(
                        this.indicators.success,
                        HtmlUtils.interpolateHtml(
                            gettext('You have applied for teacher\'s account.'),  // eslint-disable-line max-len
                            {
                            }
                        )
                    );
