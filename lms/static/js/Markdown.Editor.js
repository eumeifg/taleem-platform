...
(function() {
    ...
    var imageDialogText = gettext('Insert File (upload file or type URL)'),
        ...
        imageDescriptionHelpText = gettext("e.g. 'Sky with clouds'. The description is used as the link text."),  // eslint-disable-line max-len
    ...
    function UIManager(postfix, panels, undoManager, previewManager, commandManager, helpOptions, imageUploadHandler) {
        ...
        function makeSpritedButtonRow() {
            ...
            buttons.image = makeButton('wmd-image-button', gettext('Upload (Ctrl+G)'), '-100px', bindCommand(function(chunk, postProcessing) {
        ...
        if (chunk.endTag.length > 1 && chunk.startTag.length > 0) {
            ...
            var linkEnteredCallback = function(link, description) {
                ...
                if (link !== null) {
                    ...
                    var validImage = /(https?:\/\/.*\.(?:jpg|jpeg|gif|bmp|png|tiff))/i.test(link);
                    chunk.startTag = isImage && validImage ? '![' : '[';
                    chunk.endTag = '][' + num + ']';
