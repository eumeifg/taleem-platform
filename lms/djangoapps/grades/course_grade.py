...
class CourseGradeBase(object):
    ...
    @lazy
    def chapter_points(self):
        """
        Returns a dictionary of chapter and points.
        Here key is the chapter's usage_key.
        Value is the dictionary containing possible and
        earned points.
        """
        course_structure = self.course_data.structure
        points = OrderedDict()
        for chapter_key in course_structure.get_children(self.course_data.location):
            points[chapter_key] = self._get_chapter_points(course_structure[chapter_key], course_structure)
        return points
    ...
    def _get_chapter_points(self, chapter, course_structure):
        """
        Helper that returns a dictionary of chapter points information.
        """
        possible = 0.0
        earned = 0.0
        chapter_subsection_grades = self._get_subsection_grades(course_structure, chapter.location)

        for section in chapter_subsection_grades:
            earned += section.graded_total.earned
            possible += section.graded_total.possible
