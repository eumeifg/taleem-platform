...
from lms.djangoapps.instructor.utils import set_staff_score
...
def override_problem_score(request, course_id):
    course_key = CourseKey.from_string(course_id)
    score = int(strip_if_string(request.POST.get('score')))
    max_score = int(strip_if_string(request.POST.get('max_score')))
    submission_uuid = strip_if_string(request.POST.get('submission_uuid'))
    problem_to_reset = strip_if_string(request.POST.get('problem_to_reset'))
    max_score_show = int(strip_if_string(request.POST.get('max_score_show')))
    student_identifier = request.POST.get('unique_student_identifier', None)
    ...
    try:
        if max_score_show:
            score = (score/max_score_show) * max_score
        if submission_uuid:
            # Note: This is custom code to score the openassessment xblock from
            set_staff_score(
                course_id,
                problem_to_reset,
                earned_score=round(score),
                max_score=max_score,
