...
class IDVerificationService(object):
    ...
    @classmethod
    def user_is_verified(cls, user, earliest_allowed_date=None, is_taleem_user=False):
        """
        ...
        from openedx.custom.verification.models import PhotoVerificationVerifiedStatus

        filter_kwargs = {
            'user': user,
            'created_at__gte': (earliest_allowed_date or earliest_allowed_verification_date())
        }
        verified_state_config = PhotoVerificationVerifiedStatus.current()
        verified_statuses = []
        if verified_state_config.status == PhotoVerificationVerifiedStatus.STATUS.approved:
            verified_statuses = ['approved']
        elif verified_state_config.status == PhotoVerificationVerifiedStatus.STATUS.approved_by_ai:
            verified_statuses = ['approved', 'approved_by_ai']
        elif verified_state_config.status == PhotoVerificationVerifiedStatus.STATUS.submitted:
            verified_statuses = ['submitted', 'ready', 'approved_by_ai', 'must_retry', 'created', 'denied', 'approved']

        if is_taleem_user:
            filter_kwargs['status__in'] = verified_statuses
        else:
    ...
    def user_has_valid_or_pending(cls, user):
        ...
        filter_kwargs = {
            'user': user,
            'status__in': ['submitted', 'approved', 'approved_by_ai', 'must_retry'],
            'created_at__gte': earliest_allowed_verification_date()
        }
    ...
    def user_status(cls, user):
        ...
        elif attempt.status == 'approved_by_ai':
            user_status['status'] = 'approved_by_ai'
    ...
    @classmethod
    def get_recent_verification_for_user(cls, user):
        filter_kwargs = {
            'user': user,
        }
