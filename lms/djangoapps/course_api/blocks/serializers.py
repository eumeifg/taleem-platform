...
from .transformers.free_chapters import FreeChapterTransformer
from .transformers.block_counts import BlockCountsTransformer
from .transformers.block_points import BlockPointsTransformer
...
SUPPORTED_FIELDS = [
    ...
    SupportedFieldType(
        BlockCompletionTransformer.RESUME_BLOCK,
        BlockCompletionTransformer,
        'resume_block',
        default_value=False,
    ),
    SupportedFieldType(
        BlockPointsTransformer.BLOCK_POINTS,
        BlockPointsTransformer,
        'points',
    ),
]
...
class BlockSerializer(serializers.Serializer):  # pylint: disable=abstract-method
    ...
    def to_representation(self, block_key):
        ...
        has_access = self._get_field(
            block_key,
            FreeChapterTransformer,
            FreeChapterTransformer.HAS_ACCESS,
            True,
        )
        data.update({
        ...
        if has_access:
            data['student_view_url'] = reverse(
                'render_xblock',
                kwargs={'usage_key_string': six.text_type(block_key)},
                request=self.context['request'],
            )
            data[StudentViewTransformer.STUDENT_VIEW_DATA] = self._get_field(
                block_key,
