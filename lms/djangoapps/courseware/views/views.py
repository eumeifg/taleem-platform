...
from openedx.custom.payment_gateway.utils import (
    ensure_user_ip_in_whitelist,
    is_user_allowed_to_access_the_course,
    get_course_price
)
from openedx.custom.payment_gateway.models import CoursePrice
from openedx.custom.taleem.utils import get_course_ratings
from openedx.custom.taleem_search.utils import get_pagination_data, get_filters_data
...
from openedx.core.djangoapps.user_authn.cookies import _create_jwt
...
def courses(request):
    ...
    is_pagination, total_num_of_pages, courses_list = get_pagination_data(request, courses_list)
    ...
    return render_to_response(
        "courseware/courses.html",
        {
            ...
            'course_ratings': get_course_ratings(request.user, courses_list),
...
@ensure_user_ip_in_whitelist(allow_non_logged_in_users=False)
...
class CourseTabView(EdxFragmentView):
    ...
    @method_decorator(ensure_user_ip_in_whitelist(allow_non_logged_in_users=True, is_for_course_outline=True))
            ...
            if not CourseEnrollment.is_enrolled(request.user, course.id) and not allow_anonymous:
                ...
                if CourseTabView.course_open_for_learner_enrollment(course):
                    enroll_message = _(u'You must be enrolled in the course to see course content. '
                                       u'{enroll_link_start}Enroll now{enroll_link_end}.')
                    PageLevelMessages.register_warning_message(
                        request,
...
def course_about(request, course_id):
    ...
    with modulestore().bulk_operations(course_key):
        ...
        staff_access = bool(has_access(request.user, 'instructor', course)) or \
            bool(has_access(request.user, 'staff', course))
        studio_url = get_studio_url(course, 'settings/details')

        user_agent = get_user_agent(request)
        seb_settings = configuration_helpers.get_value('TA3LEEM_BROWSER', {})
        is_tb = not seb_settings or "Ta3leem/2.0" in user_agent.ua_string
        browser_download_link = seb_settings.get('EXE', None)
        config_download_link = seb_settings.get('CONFIG_FILE', None)
        course_target = reverse(course_home_url_name(course.id), args=[text_type(course.id)])
        course_target = (staff_access or is_tb) and course_target or config_download_link
        ...
        try:
            taleem_course_price = get_course_price(course_id)

            # Use discount from the vouchers user has already applied, given that user is authenticated
            if request.user.is_authenticated:
                discount_price = taleem_course_price.get_course_price_for_user(request.user)
            else:
                discount_price = taleem_course_price.price

            if discount_price.is_zero():
                course_price = _('Free')
                registration_price = 0
            else:
                course_price = _('{currency_symbol}{price}').format(
                    currency_symbol=taleem_course_price.currency.upper(), price=discount_price
                )
                registration_price = discount_price
        except CoursePrice.DoesNotExist:
            taleem_course_price = None
        ...
        context = {
            ...
            'is_tb': is_tb,
            'browser_download_link': browser_download_link,
            ...
            'is_user_allowed_to_access_the_course': is_user_allowed_to_access_the_course(request, course_id),
            'taleem_course_price': taleem_course_price,
            'vouchers_page_url': reverse('payment_gateway:vouchers_page', args=(text_type(course_id), )),
        }
...
def course_exams(request, course_id):
    """
    Display the course's exams.html, or 404 if there is no such course.
    Assumes the course_id is in a valid format.
    """
    course_key = CourseKey.from_string(course_id)
    ...
    context = {
        'course': course,
        'exams': TimedExam.objects.filter(key__in=exam_keys),
        'supports_preview_menu': True,
        'can_masquerade': can_masquerade,
        'masquerade': masquerade,
        'on_exams_tab': True,
    ...
    return render_to_response('courseware/exams.html', context)
...
@ensure_csrf_cookie
@api_view(['GET'])
@authentication_classes([SessionAuthentication, JwtAuthentication, BearerAuthenticationAllowInactiveUser])
def render_xblock(request, usage_key_string, check_if_enrolled=True):
    ...
    with modulestore().bulk_operations(course_key):
        ...
        while parent.category not in ['chapter', 'course']:
            parent = parent.get_parent()
        
        block_types_filter = ['chapter']

        course_usage_key = modulestore().make_course_usage_key(course_key)
        all_blocks = get_blocks(request,course_usage_key,user=request.user,nav_depth=3,\
            requested_fields=[
            'display_name',
            'type',
            'format'
        ...
        first_three_chapters = list(all_blocks['blocks'].keys())[:3]
        if request.user.is_anonymous and  parent.category == 'chapter' and str(parent.scope_ids.usage_id) not in first_three_chapters:
        ...
        student_view_context['is_rendering_xblock'] = True

        enable_completion_on_view_service = False
        completion_service = block.runtime.service(block, 'completion')
        if completion_service and completion_service.completion_tracking_enabled():
            if block.visual_completion == 'default' and completion_service.blocks_to_mark_complete_on_view({block}):
        ...
        context = {
            ...
            'access_token': 'DUMMY' if request.user.is_anonymous else _create_jwt(request, request.user, settings.JWT_AUTH['JWT_IN_COOKIE_EXPIRATION']),
        }
