...
from openedx.custom.payment_gateway.utils import ensure_user_ip_in_whitelist
...
class CoursewareIndex(View):
    ...
    @method_decorator(ensure_user_ip_in_whitelist(allow_non_logged_in_users=True, is_for_courseware=True))
    ...
    def _is_monitoring_enable(self):
        from openedx.custom.timed_exam.models import TimedExam
        return TimedExam.is_monitored_timed_exam(self.course_key)
    ...
    def _create_courseware_context(self, request):
        ...
        course_instructor = CourseAccessRole.objects.filter(
            org=self.course.org,
        ...
        courseware_context = {
            ...
            'instructor_name': course_instructor.user.username if course_instructor else None,
            'instructor_email': course_instructor.user.email if course_instructor else None,
            'init': '',
            'fragment': Fragment(),
            ...
            'is_monitoring_enable': self._is_monitoring_enable()
        }
        ...
        if not request.user.is_authenticated or not CourseEnrollment.is_enrolled(request.user, self.course.id):
            table_of_contents['chapters'] = table_of_contents['chapters'][:3]
