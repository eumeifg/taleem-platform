...
def get_module_system_for_user(
    ...
    def get_event_handler(event_type):
        ...
        handlers = {
            ...
            'survey_submitted': handle_course_survey_submitted
        }
    ...
    def handle_course_survey_submitted(block, event):
        """
        Initiate the process of notification for user.
        """
        course = modulestore().get_course(block.course_id, depth=0)
        course_author = CourseAccessRole.objects.select_related('user').filter(
            course_id=text_type(block.course_id)
            ).first()
        if course_author and block.send_notifications and block.runtime.user_id:
            student = User.objects.get(id=block.runtime.user_id)
            notify_user(
                user=course_author.user,
                notification_type=NotificationTypes.COURSE_SURVEY_SUBMITTED,
                notification_message="{{student:{student}}} submitted a survey for course {{course_name:{course_name}}}".format(
