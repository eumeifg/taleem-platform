...
def forum_form_discussion(request, course_key):
    ...
    if request.is_ajax():
        ...
        try:
            ...
            is_staff = has_permission(request.user, 'openclose_thread', course.id)
            is_teacher = request.user.ta3leem_profile.user_type == UserType.teacher.name
            course_staff_or_instructor = CourseAccessRole.objects.filter(
                user=request.user,
            ...
            threads = [
                ...
                if any((
                    ...
                    is_staff,
                    is_teacher and course_staff_or_instructor,
                ))
            ]
