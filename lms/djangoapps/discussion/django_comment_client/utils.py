...
def get_ability(course_id, content, user):
    ...
    if content['type'] == 'thread' and content['thread_type'] == 'private':
        can_reply = any((
            user.is_superuser,
            content.get('username') == user.username,
            user.ta3leem_profile.can_answer_discussion,
            has_permission(user, 'openclose_thread', course_id),
        ))
    else:
