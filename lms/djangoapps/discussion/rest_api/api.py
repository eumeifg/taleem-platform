...
def _serialize_discussion_entities(request, context, discussion_entities, requested_fields, discussion_entity_type):
    ...
    for entity in discussion_entities:
        if discussion_entity_type == DiscussionEntity.thread:
            if all((
                entity.get("thread_type") == "private",
                entity.get('username') != request.user.username,
                not request.user.ta3leem_profile.can_answer_discussion,
            )):
                continue
...
def get_thread_list(
    ...
    num_pages = int(math.ceil(thread_count / page_size * 1.0))
    ...
    for field, form_value in actions_form.cleaned_data.items():
        if field in request_fields and form_value != api_content[field]:
            ...
            if field == "following":
                ...
                _handle_pinned_field(form_value, cc_content, api_content, context["cc_requester"])
            elif field == "muted":
                _handle_muted_field(form_value, cc_content, api_content, context)
            else:
...
def _handle_pinned_field(form_value, thread, api_content, user):
    """pin or unpin on thread"""
    if form_value:
        api_content["pinned"] = True
        thread.pin(user, thread.id)
    else:
        api_content["pinned"] = False
        thread.un_pin(user, thread.id)


def _handle_muted_field(mute, thread, api_content, context):
    """mute/unmute thread for the user"""
