...
class _ContentSerializer(serializers.Serializer):
    ...
    def get_author_photo(self, obj):
        """
        Returns the profile pic of the content author.
        """
        if self._is_anonymous(obj) or obj["user_id"] is None:
            return None
        try:
            user = DjangoUser.objects.get(id=obj["user_id"])
            return get_profile_image_urls_for_user(
                user,
...
class ThreadSerializer(_ContentSerializer):
    ...
    type = serializers.ChoiceField(
        source="thread_type",
        choices=[(val, val) for val in ["discussion", "question", "private"]]
    )
    ...
    muted = serializers.SerializerMethodField(required=False)
    ...
    def get_muted(self, obj):
        """
        Whether the user has muted this thead for
        notifications.
        """
        return MutedPost.objects.filter(
    ...
    def get_comment_count(self, obj):
        ...
        Ta3leem: count = num of responses
        """
    ...
    def get_unread_comment_count(self, obj):
        ...
        comments.
        Ta3leem: count = num of unread responses
        """
