...
class BadgrBackend(BadgeBackend):
    ...
    def __init__(self):
        ...
        if None in (settings.BADGR_USERNAME,
            ...
            error_msg = (
                ...
                "Required settings: BADGR_USERNAME, BADGR_PASSWORD, BADGR_CLIENT_D, "
    ...
    def _create_badge(self, badge_class):
        ...
        img_file = Image.open(urlopen(image.url))
        image_file = io.BytesIO()
        img_file.save(image_file, format="PNG")
        image_file.seek(0)
    ...
    def _create_assertion(self, badge_class, user, evidence_url):
        ...
        data = {
            ...
            "evidence": [
                {
                    "url": evidence_url or settings.LMS_ROOT_URL
                }
            ],
            "notify": settings.BADGR_ENABLE_NOTIFICATIONS,
            "badgeclass": badge_class.badgr_server_slug,
        }
    ...
    def _get_and_cache_oauth_tokens(self, refresh_token=None):
        ...
        data = {
            ...
            'client_id': settings.BADGR_CLIENT_D,
            'grant_type': 'password'
        }
        if refresh_token:
            data = {
