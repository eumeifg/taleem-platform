...
class LibraryContentFields(object):
    ...
    source_library_id = String(
        ...
        help=_("Select the question bank from which you want to draw content."),
        scope=Scope.settings,
    ...
    optional_count = Integer(
        display_name=_("How many problems are optional?"),
        help=_("What is the number of optional problems."),
        scope=Scope.settings,
        default=0,
    )
    max_count = Integer(
    ...
    capa_type = String(
        ...
        help=_(
            'Choose a problem type to fetch from the question bank. If "Any Type" is selected no filtering is applied.'
        ),
    ...
    traversable = Boolean(
        display_name=_("Traversable"),
        help=_("Whether to allow to navigate through questions."),
        scope=Scope.settings,
        default=False
    )
    selected = List(
...
class LibraryContentModule(LibraryContentFields, XModule, StudioEditableModule):
    ...
    def _get_child_blocks(self):
        """
        Generator returning XBlock instances of the children.
        """
        for child in self.children:
            yield self.runtime.get_block(
                self.location.course_key.make_usage_key(child.block_type, child.block_id)
            )
...
class LibraryContentDescriptor(LibraryContentFields, MakoModuleDescriptor, XmlDescriptor, StudioEditableDescriptor):
    ...
    def validate(self):
        ...
        if not self.source_library_id:
            validation.set_summary(
                StudioValidationMessage(
                    ...
                    _(u"A question bank has not yet been selected."),
                    action_class='edit-button',
                    action_label=_(u"Select a Question Bank.")
                )
            )
            return validation
        lib_tools = self.runtime.service(self, 'library_tools')
        ...
        if self.optional_count > 0:
            unique_problem_weights = set(
                problem.weight or 1 for problem in self._get_child_blocks() if problem and hasattr(problem, 'weight')
            )
            if len(unique_problem_weights) > 1:
                self._set_validation_error_if_empty(
                    validation,
                    StudioValidationMessage(
                        StudioValidationMessage.ERROR,
                        _(u'All library problems do not have same weightage.'),
                        action_class='edit-button',
                        action_label=_(u"Select another library.")
                    )
                )
    ...
    def source_library_values(self):
        ...
        all_libraries = [(u"", _("No Question Bank Selected"))] + all_libraries
