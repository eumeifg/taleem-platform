...
from lms.djangoapps.courseware.courses import get_course_by_id, get_course_about_section
...
try:
    from openedx.custom.video_feedback.models import VideoRating, VideoLike
    rating_and_like = True
except:
    rating_and_like = None
...
# Default expiration, in seconds, of one-time URLs used for watching videos.
...
class VideoBlock(
    ...
    def prioritize_hls(self, youtube_streams, html5_sources):
        ...
        hls_present = any(source for source in html5_sources if source and source.strip().endswith('.m3u8'))
    ...
    def student_view(self, _context):
        ...
        """
        fragment = Fragment(self.get_html(xb_context=_context))
    ...
    def get_html(self, view=STUDENT_VIEW, xb_context={}):
        ...
        if self.edx_video_id and edxval_api:
            try:
                ...
                val_video_urls = edxval_api.get_urls_for_profiles(
                    self.edx_video_id.strip(), ['hls'])
                sources = [val_video_urls.get('hls', '')]
        ...
        poster = get_poster(self).get('url')
        ...
        metadata = {
            ...
            'completionEnabled': completion_enabled and visual_completion != 'disable',
        ...
        context = {
            ...
            'lms_view': rating_and_like,
            'is_rendering_xblock': xb_context.get('is_rendering_xblock', False),
            'visual_completion': visual_completion,
            'course_link': "/courses/{}/course/".format(six.text_type(course_key)),
            'done_state': getattr(self, "done", False),
            'overview': getattr(self, "overview", False),
            'course': course,
            'user_id': user_id,
        }
        if rating_and_like and user_id:
            context.update({
                ...
                'user_like': VideoLike.get_user_like(user_id, course_key, block_key),
                'user_rating': VideoRating.get_user_rating(user_id, course_key, block_key),
                'avg_rating': VideoRating.avg_rating(course_key, block_key),
                'num_reviews': VideoRating.num_reviews(course_key, block_key),
                'likes': VideoLike.total_likes(course_key, block_key),
                'user_like': VideoLike.get_user_like(user_id, course_key, block_key),
    ...
    def editable_metadata_fields(self):
        ...
        # Change overview field type to Text
        editable_fields['overview']['type'] = 'Text'
    ...
    def get_context(self):
        ...
        if self.edx_video_id and edxval_api:
            val_video_urls = edxval_api.get_urls_for_profiles(
                self.edx_video_id, ['hls'])
            source_url = val_video_urls.get('hls', '')
    ...
    def student_view_data(self, context=None):
        ...
        if self.youtube_disabled_for_course():
            ...
            self.youtube_streams = create_youtube_string(self)  # pylint: disable=W0201
        ...
        return {
            ...
            "thumbnail": get_poster(self).get('url'),
        }
