...
def get_poster(video):
    ...
    if video.youtube_streams:
        ...
        url = None
        if edxval_api and video.edx_video_id:
            url = edxval_api.get_course_video_image_url(
                course_id=video.runtime.course_id.for_branch(None),
                edx_video_id=video.edx_video_id.strip()
            )
        poster["url"] = url or "https://storage.ta3leem.iq/static/poster.png"
        poster["type"] = "html5"
