...
(function() {
    ...
    this.Problem = (function() {
        ...
        Problem.prototype.autosave = function() {
            var that = this;
            // document.getElementsByClassName('exam-timer') is added for restricting the autosave functionality
            // for only timed exam removing this check will start doing same in the course.
            if (that.saveButton.length && !that.url.startsWith('/preview/xblock')) {
                setInterval(function() {
                    if (document.getElementsByClassName('exam-timer').length &&
                        that.el.closest(".exam-problem").is(":visible") &&
                      (!that.el.find('.status').hasClass('correct') && !that.submitButton.attr('disabled'))) {
