...
(function() {
    ...
    this.Sequence = (function() {
        ...
        Sequence.prototype.updatePageTitle = function() {
            // update unit title
            $(".unit-title").text($(".seq_problem.nav-item.tab.active").data("page-title"));
        ...
        Sequence.prototype.addToUpdatedProblems = function(problemId, newContentState, newState) {
            ...
            */
            ...
            // At last, change next to skip if applicable
            if (this.$(".skip-enabled").length > 0) {
                this.$(".skip-enabled").removeClass('can-skip');
                $("#exam-button-text").text(gettext('Next'));
            }
        };
        ...
        Sequence.prototype.selectNext = function(event) {
            var self = this;
            // Show a warning message if it is an exam and
            // student wants to skip the question
            if ($(".exam-button-next").hasClass('can-skip')) {
                if (typeof swal !== "undefined") {
                    swal({
                        title: gettext("Are you sure?"),
                        text: gettext("Once done, you will not be able to change it later!"),
                        icon: "warning",
                        buttons: [gettext("Cancel"), gettext("OK")],
                        dangerMode: true,
                    })
                    .then(function(willProceed) {
                        if (willProceed == true) {
                            self._change_sequential('next', event);
                        }
                    });
                } else {
        ...
        Sequence.prototype._change_sequential = function(direction, event) {
            ...
            // Change label of skip button to next
            if (direction == 'next') {
                $(".exam-button-next.skip-enabled").addClass('can-skip')
                $("#exam-button-text").text(gettext('Skip'));
            }
        };
