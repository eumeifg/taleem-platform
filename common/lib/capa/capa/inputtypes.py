...
class OptionInput(InputTypeBase):
    ...
    def parse_options(options):
        ...
        options = [(t, t) for t in tokens]
        try:
            seed = get_current_user().id
        except Exception:
            # This block is here for extra precaution and if get_current_user
            # raise an exception then we will use the default seed
            seed = BY_DEFAULT_SEED
        random.seed(seed)
...
class ChoiceGroup(InputTypeBase):
    ...
    def setup(self):
        ...
        try:
            seed = str(get_current_user().id) + self.input_id
        except Exception:
            # This block is here for extra precaution and if get_current_user
            # raise an exception then we will use the default seed
            seed = BY_DEFAULT_SEED
