...
from openedx.core.djangoapps.theming.helpers import get_config_value_from_site_or_settings, get_themes
...
def generate_activation_email_context(user, registration):
    ...
    if settings.FEATURES['ENABLE_MKTG_SITE']:
        ...
        contact_url = '{protocol}://{site}{link}'.format(
...
def do_create_account(form, custom_form=None):
    ...
    try:
        with transaction.atomic():
            user.save()
            if custom_form:
                custom_model = custom_form.save(commit=False, user=user)
