...
class UserProfile(models.Model):
    ...
    VALID_YEARS = list(range(this_year, this_year - 100, -1))
    year_of_birth = models.IntegerField(blank=True, null=True, db_index=True)
    GENDER_CHOICES = (
        (u'm', ugettext_noop(u'Male')),
        (u'f', ugettext_noop(u'Female')),
        # Translators: 'Other' refers to the student's gender
        (u'o', ugettext_noop(u'Prefer Not to Say'))
    )
    ...
    LEVEL_OF_EDUCATION_CHOICES = (
        (u'p', ugettext_noop(u"PhD")),
        (u'm', ugettext_noop(u"Master's degree")),
        (u'hd', ugettext_noop(u"High Diploma")),
        (u'b', ugettext_noop(u"Bachelor's degree")),
        (u'a', ugettext_noop(u"Diploma")),
        (u'hs', ugettext_noop(u"High School")),
        (u'jhs', ugettext_noop(u"Intermediate School")),
        (u'el', ugettext_noop(u"Primary School")),
        (u'pc', ugettext_noop(u"Professional Certificate")),
        # Translators: 'None' refers to the student's level of education
        (u'none', ugettext_noop(u"No Certificates")),
    ...
    FIRST_DAY_OF_WEEK_CHOICES = (
        (u'mon', ugettext_noop(u"Monday")),
        (u'tue', ugettext_noop(u"Tuesday")),
        (u'wed', ugettext_noop(u"Wednesday")),
        (u'thu', ugettext_noop(u"Thursday")),
        (u'fri', ugettext_noop(u"Friday")),
        (u'sat', ugettext_noop(u"Saturday")),
        (u'sun', ugettext_noop(u"Sunday"))
    )
    DATE_FORMAT_CHOICES = (
        (u'num', ugettext_noop(u"MM/DD/YYYY")),
        (u'short', ugettext_noop(u"MMM DD, YYYY")),
        (u'long', ugettext_noop(u"MMMM DD, YYYY"))
    )
    RECEIVE_REMINDER_NOTIFICATIONS_CHOICES = (
        (u'true', ugettext_noop(u"True")),
        (u'false', ugettext_noop(u"False"))
    )
    mailing_address = models.TextField(blank=True, null=True)
    city = models.TextField(blank=True, null=True)
    country = CountryField(blank=True, null=True)
    COUNTRY_WITH_STATES = u'IQ'
    STATE_CHOICES = (
        ('AN','Al Anbar'),
        ('BB','Babylon'),
        ('BG','Baghdad'),
    ...
    state = models.CharField(blank=True, null=True, max_length=20, choices=STATE_CHOICES)
    ...
    external_user_id = models.IntegerField(blank=True, null=True, db_index=True, help_text=_("Reference to user from external SIS module."))
    ...
    @property
    def new_field_data(self):
        # """ Convenience method that returns the human readable gender. """
        # if self.gender:
            # return self.__enumerable_to_display(self.GENDER_CHOICES, self.gender)
        return "Arsalan testing here"
...
class CourseEnrollment(models.Model):
    ...
    @classmethod
    def enrollments_for_user(cls, user, timed_exam=False):
        return cls.objects.filter(
            user=user, is_active=1, course__is_timed_exam=timed_exam
        ).select_related('user')
    ...
    def enrollments_for_user_with_overviews_preload(cls, user, courses_limit=None):  # pylint: disable=invalid-name
        ...
        """
        enrollments = cls.enrollments_for_user(user).select_related(
            'schedule', 'course', 'course__image_set'
        )
