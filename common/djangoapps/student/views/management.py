...
def index(request, extra_context=None, user=AnonymousUser()):
    ...
    is_pagination, total_num_of_pages, courses = get_pagination_data(request, courses)
    ...
    context['course_ratings'] = get_course_ratings(request.user, courses)
    ...
    return render_to_response('index-opt.html', context)
...
def change_enrollment(request, check_access=True):
    ...
    if not is_user_allowed_to_access_the_course(request, course_id):
        return HttpResponseBadRequest(
            _(
                "User is not allowed to enroll in this course. "
                "Please either buy this course or contact your administrator for the access."
            )
        )
