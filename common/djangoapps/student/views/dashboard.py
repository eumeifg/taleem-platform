...
from openedx.custom.live_class.api.courses import my_live_classes
from openedx.custom.taleem.utils import get_course_ratings
from openedx.custom.taleem.models import UserType
from openedx.custom.timed_exam.models import TimedExam, TimedExamHide
...
def get_exam_enrollments(user):
    """
    Given a user, return a filtered set of his or her exam enrollments.

    Arguments:
        user (User): the user in question.

    Returns:
        past and upcoming list[CourseEnrollment]:
        a sequence of enrollments to be displayed
    ...
    enrolled_keys = [
        ...
        for enrollment in CourseEnrollment.enrollments_for_user(
            user, timed_exam=True
        )
    ]
    hidden_keys = TimedExamHide.objects.filter(
        user=user
    ).values_list('timed_exam__key', flat=True)
    exams = TimedExam.objects.filter(
        key__in=enrolled_keys
    ).exclude(key__in=hidden_keys)
    for exam in exams:
    ...
    return past_exams, upcoming_exams
...
def get_verification_error_reasons_for_display(verification_error_codes):
    ...
    verification_error_map = {
        'photos_mismatched': 'Photos are mismatched',
        'id_image_missing_name': 'Name missing from ID photo',
        'id_image_missing': 'ID photo not provided',
        'id_invalid': 'ID is invalid',
        'user_image_not_clear': 'Learner photo is blurry',
        'name_mismatch': 'Name on ID does not match name on account',
        'user_image_missing': 'Learner photo not provided',
        'id_image_not_clear': 'ID photo is blurry',
    }
...
def get_co_teacher_exams(user):
    if user.is_superuser or user.is_staff:
        return []

    is_teacher = user.ta3leem_profile.user_type == UserType.teacher.name
    if is_teacher and not user.ta3leem_profile.can_create_exam:
        return []

    return TimedExam.objects.filter(
        key__in=CourseAccessRole.objects.filter(
            user=user,
...
def student_dashboard(request):
    ...
    # Get the timed exams for the user
    past_exams, upcoming_exams = get_exam_enrollments(user)

    # Sort the exam enrollment pairs by the enrollment due date
    past_exams.sort(key=lambda x: x.due_date, reverse=True)
    upcoming_exams.sort(key=lambda x: x.due_date)
    exam_enrollments = upcoming_exams + past_exams

    # Get Live classes for the user
    live_classes = my_live_classes(user)
    ...
    if not user.is_active:
        activate_account_message = Text(_(
            "{paragraph_act_email_send_msg_start}Check your {email_start}{email}{email_end} inbox for an account activation link from {platform_name}.{paragraph_act_email_send_msg_end} If you do not receive an email {link_send_act_email_start} Click Here To Resend {link_send_act_email_end}."
            "If you need help, contact {link_start}{platform_name} Support{link_end}."
        )).format(
            paragraph_act_email_send_msg_start=HTML(u'<span id="dashboard_activation_email_send_msg">'),
            paragraph_act_email_send_msg_end=HTML(u'</span>'),
            platform_name=platform_name,
            ...
            link_end=HTML("</a>"),
            link_send_act_email_start=HTML(u'<a id="resend_activation_email" href="#">'),
            link_send_act_email_end=HTML("</a>"),
        )
    ...
    elif 'tashgeel_auth' in request.GET:
        redirect_message = request.GET['tashgeel_auth']
    elif 'notverified' in request.GET:
        redirect_message = _("You need to complete the ID verification first for accessing the studio.")
    else:
        redirect_message = ''

    valid_verification_statuses = ['approved', 'approved_by_ai', 'must_reverify', 'pending', 'expired']
