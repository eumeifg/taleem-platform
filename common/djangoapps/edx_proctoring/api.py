...
from openedx.custom.notifications.utils import send_exam_submit_notification
from openedx.custom.timed_exam.models import TimedExam, TimedExamAlarms
from openedx.custom.timed_exam.utils import mark_timed_exam_attempt
...
def _start_exam_attempt(existing_attempt):
    ...
    # Log the exam start
    ProctoredExamSessionConnectionHistory.log_start_time(
        existing_attempt.user,
        existing_attempt.proctored_exam.course_id,
    )
...
def update_attempt_status(exam_id, user_id, to_status,
    ...
    # This is the custom code for Ta3leem to verify the images for web monitoring.
    # Also triggers grade calculation
    if to_status in [ProctoredExamStudentAttemptStatus.timed_out, ProctoredExamStudentAttemptStatus.submitted]:
        from openedx.custom.timed_exam.tasks import verify_timed_exam_snapshots
        verify_timed_exam_snapshots.delay(
    ...
    # Mark user's attempt in TimedExamAttempt table.
    request = get_current_request()
    ip_address = get_ip(request) if request else '0.0.0.0'
    mark_timed_exam_attempt(exam_attempt_obj.user, exam_attempt_obj.proctored_exam.course_id, ip_address)

    if exam_attempt_obj.started_at:
        # Setting the Timed exam alerts for students.
        TimedExamAlarms.set_alarms(exam_attempt_obj)

    if to_status == ProctoredExamStudentAttemptStatus.submitted:
        send_exam_submit_notification(user=exam_attempt_obj.user, exam_attempt=exam_attempt_obj)
...
def _get_timed_exam_view(exam, context, exam_id, user_id, course_id):
    ...
    if not attempt_status:
    ...
    elif attempt_status == ProctoredExamStudentAttemptStatus.enrollment_code_required:
        if time_exam and time_exam.generate_enrollment_code and not attempt['allow_entrance']:
            student_view_template = 'timed_exam/add_enrollment_code.html'
        else:
            student_view_template = 'timed_exam/entrance.html'
        ...
        student_view_template = 'timed_exam/online_exam.html'
        context.update({
            'problems': get_assigned_problems(
                request,
                user_id,
                course_id,
            )
        })
    elif attempt_status == ProctoredExamStudentAttemptStatus.not_verified:
        # allow everyone boolean enforcing the entrance in case system wide
        # setting is allow everyone PhotoVerificationVerifiedStatus.allow
        approved_statuses, allow_everyone = get_approved_statuses(time_exam)
        if context.get('verification_status') not in approved_statuses and not allow_everyone:
            # if the user has not id verified yet, show them the page that requires them to do so
            student_view_template = 'timed_exam/id_verification.html'
        else:
            student_view_template = 'timed_exam/entrance.html'
    elif attempt_status == ProctoredExamStudentAttemptStatus.no_verification_need:
        if context.get('verification_status') == 'none':
            student_view_template = 'timed_exam/id_verification.html'
        else:
            student_view_template = 'timed_exam/entrance.html'
    ...
    if student_view_template:
        ...
        context.update({
            ...
            'enrollment_code': time_exam.enrollment_code,
