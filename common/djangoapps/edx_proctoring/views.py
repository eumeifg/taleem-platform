...
class StudentProctoredExamAttempt(ProctoredAPIView):
    ...
    **Response Values**
    ...
    def get(self, request, attempt_id):
        ...
        # Log the poll time
        ProctoredExamSessionConnectionHistory.log_poll_time(
            request.user,
            attempt_id,
        )

        # Attempt modified, fetch it again
        attempt = get_exam_attempt_by_id(attempt_id)
    ...
    **Response Values**
    ...
    def get(self, request):  # pylint: disable=unused-argument
        ...
        if exams:
            ...
            # Get Ta3leem Exam
            taleem_exam = TimedExam.get_obj_by_course_id(exam['course_id'])
    ...
    def post(self, request):
        ...
        is_verified = IDVerificationService.user_is_verified(request.user, is_taleem_user=True)
        ...
        attempt_obj = ProctoredExamStudentAttempt.objects.get_exam_attempt(exam_id, request.user)
        if attempt_obj and (attempt_obj.status == ProctoredExamStudentAttemptStatus.not_verified or
                            attempt_obj.status == ProctoredExamStudentAttemptStatus.no_verification_need or
                            attempt_obj.status == ProctoredExamStudentAttemptStatus.enrollment_code_required):
            exam_attempt_id = attempt_obj.id
        else:
            ...
            attempt_obj = ProctoredExamStudentAttempt.objects.get(id=exam_attempt_id)

        timed_exam = TimedExam.get_obj_by_course_id(attempt_obj.proctored_exam.course_id)
        photo_verification_status_class = timed_exam.get_timed_exam_verification_status_class()
        verified_state_config = photo_verification_status_class.current()
        if verified_state_config.status == photo_verification_status_class.STATUS.allow:
            is_verified = True
        ...
        elif is_timed_exam and not is_verified:
            # this is custom change for ta3leem, if user is not verified then
            # we are changing the attempt status so we can stop the learner.
            timed_exam_status = ProctoredExamStudentAttemptStatus.not_verified
            if verified_state_config.status == photo_verification_status_class.STATUS.submitted:
                timed_exam_status = ProctoredExamStudentAttemptStatus.no_verification_need
            update_attempt_status(
                exam_id,
                request.user.id,
                timed_exam_status
            )
        elif is_timed_exam and timed_exam and timed_exam.generate_enrollment_code and not attempt_obj.allow_entrance:
            timed_exam_status = ProctoredExamStudentAttemptStatus.enrollment_code_required
            update_attempt_status(
                exam_id,
                request.user.id,
                timed_exam_status
...
class TabSwitchHistory(AuthenticatedAPIView):
    """
    Log the time when user lefts or returns to the
    exam tab.
    """
    def post(self, request, course_id):  # pylint: disable=unused-argument
        """ Log in/out for the given user and course id """
        event_type = request.POST.get('event_type', 'in')
        attempt_id = request.POST.get('attempt_id')
        ProctoredExamTabSwitchHistory.log_event(request.user, course_id, event_type)
        # Update active time
        ProctoredExamSessionConnectionHistory.log_poll_time(
            request.user,
            attempt_id,
...
class ProctoredExamReviewStatus(AuthenticatedAPIView):
    """
    Proctored exam review.
    """
    def post(self, request, course_id):  # pylint: disable=unused-argument
        """ Make a review for the given users and course id """
        # Get the course key
        try:
        ...
        review_status = params.get('status', 'clean')
        clean_only = 'status' not in params
        student_ids = params.get('ids', [])

        if not review_status:
            return HttpResponseBadRequest()

        ProctoredExamReview.make_a_review(student_ids, course_key, review_status, clean_only)
        return Response(status=status.HTTP_200_OK)
