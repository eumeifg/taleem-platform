...
define(['jquery', 'gettext', 'common/js/components/utils/view_utils', 'js/views/utils/create_utils_base'],
    function($, gettext, ViewUtils, CreateUtilsFactory) {
        ...
        return function(selectors, classes) {
             ...
             * Check that a course (org, number, run) doesn't use any special characters but allows spaces.
            */
            var self = this;
            this.validateURLItemEncoding = function (item, allowUnicode) {
               var required = self.validateRequiredField(item);
                if (required) {
                    return required;
                }
                if (item.match(/[!'()*]/)) {
                  return gettext('Please do not use any special characters in this field.');
                }
                return '';
            };
            ...
            this.fetchColleges = function(university, successHandler, errorHandler) {
                $.getJSON(
                    '/taleem-organization/colleges/' + university + '/'
                ).done(function(data) {
                    successHandler(data);
                }).fail(function(jqXHR, textStatus, errorThrown) {
                    var reason = errorThrown;
            ...
            this.fetchDepartments = function(university, college, successHandler, errorHandler) {
                $.getJSON(
                    '/taleem-organization/departments/' + university + '/' + college + '/'
                ).done(function(data) {
                    successHandler(data);
                }).fail(function(jqXHR, textStatus, errorThrown) {
                    var reason = errorThrown;
