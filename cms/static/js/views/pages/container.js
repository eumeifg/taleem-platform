    ...
    function($, _, Backbone, gettext, BasePage, ViewUtils, ContainerView, XBlockView, AddXBlockComponent,
        ...
        var XBlockContainerPage = BasePage.extend({
            ...
            events: {
                ...
                'click .settings-button': 'xBlockSettings',
            ...
            deleteComponent: function(xblockElement) {
                ...
                if (this.isLibraryPage && $('.action-delete').length === 1) {
                    $.ajax({
                        url: '/can_delete_xblock/' + xblockInfo.id + '/',
                        type: 'POST',
                        success: function(response) {
                            if (response.can_delete) {
                              ...
                              // eslint-disable-next-line max-len
                                window.alert("You can't delete the last problem from the question bank, if it is assigned to any exam");
                            }
                        }
                    });
                } else {
