...
from cms.djangoapps.contentstore.constants import BLACKLISTED_PROBLEMS
...
def is_problem_filtered(problem_template, problem_blacklist=BLACKLISTED_PROBLEMS):
    """
    Check if the given problem template should be allowed or not.
    """
    template_id = problem_template.get('template_id')
    return any(problem in template_id for problem in problem_blacklist)


def user_in_group(user, group):
    """
    Check if given user is present in the given group.
    """
    return user.groups.filter(name=group).exists()


def add_items_in_module_list(items, module_descriptor):
    """
    Add items to the given descriptor advanced module list.
    module_descriptor can be either course or library.
    """
    list_updated = False
    for item in items:
        if item not in module_descriptor.advanced_modules:
            module_descriptor.advanced_modules.append(item)
            list_updated = True
    return list_updated
