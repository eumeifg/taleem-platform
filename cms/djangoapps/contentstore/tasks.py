...
from openedx.custom.question_bank.csv_importer import import_questions_from_csv
from openedx.custom.question_bank.csv_exporter import export_questions_to_csv
...
@task(base=CourseImportTask, bind=True)
def import_csv(self, user_id, course_key_string, archive_path, archive_name, language):
    """
    Import questions from a provided CSV.
    """
    courselike_key = CourseKey.from_string(course_key_string)
    try:
        user = User.objects.get(pk=user_id)
    ...
    # Locate the uploaded CSV file (and download it from S3 if necessary)
    # Do everything in a try-except block to make sure everything is properly cleaned up.
    data_root = path(settings.GITHUB_REPO_ROOT)
    subdir = base64.urlsafe_b64encode(repr(courselike_key).encode('utf-8')).decode('utf-8')
    course_dir = data_root / subdir
    try:
        ...
        if not archive_name.endswith(u'.csv'):
            with translation_language(language):
                self.status.fail(_(u'We only support uploading a .csv file.'))
                return
        ...
        LOGGER.debug(u'importing CSV to {0}'.format(temp_filepath))

        # Copy the OLX archive from where it was uploaded to (S3, Swift, file system, etc.)
        if not course_import_export_storage.exists(archive_path):
            LOGGER.info(u'CSV import %s: Uploaded file %s not found', courselike_key, archive_path)
            with translation_language(language):
                self.status.fail(_(u'CSV file not found'))
            return
        with course_import_export_storage.open(archive_path, 'rb') as source:
            with open(temp_filepath, 'wb') as destination:
                def read_chunk():
                ...
                for chunk in iter(read_chunk, b''):
                    destination.write(chunk)
        LOGGER.info(u'CSV import %s: Download from storage complete', courselike_key)
        # Delete from source location
        course_import_export_storage.delete(archive_path)
        LOGGER.info(u'CSV import %s: Uploaded file deleted from source', courselike_key)
    # Send errors to client with stage at which error occurred.
    except Exception as exception:  # pylint: disable=broad-except
        if course_dir.isdir():
            shutil.rmtree(course_dir)
            LOGGER.info(u'CSV import %s: Temp data cleared', courselike_key)

        LOGGER.exception(u'Error importing CSV %s', courselike_key, exc_info=True)
        self.status.fail(text_type(exception))
        return
    ...
    try:
        ...
        LOGGER.info(u'CSV import %s: file verified', courselike_key)
        self.status.set_state(u'Updating')
        self.status.increment_completed_steps()

        import_questions_from_csv(
            user_id,
            courselike_module.location,
            temp_filepath,
        )

        LOGGER.info(u'CSV import %s: Questions import successful', courselike_key)
    except Exception as exception:   # pylint: disable=broad-except
        LOGGER.exception(u'error importing csv', exc_info=True)
        self.status.fail(text_type(exception))
    finally:
        if course_dir.isdir():
            shutil.rmtree(course_dir)
...
@task(base=CourseExportTask, bind=True)
def export_csv(self, user_id, course_key_string, language):
    """
    Export questions to a CSV and prepare it for download.
    """
    courselike_key = CourseKey.from_string(course_key_string)
    ...
    question_bank = modulestore().get_library(courselike_key)

    try:
        self.status.set_state(u'Exporting')
        csv_file = create_export_csv(question_bank, courselike_key, {}, self.status)
        csv_file_content = BytesIO(csv_file.read().encode('utf-8'))
        artifact = UserTaskArtifact(status=self.status, name=u'Output')
        artifact.file.save(name=os.path.basename(csv_file.name), content=csv_file_content)
        artifact.save()
    # catch all exceptions so we can record useful error messages
    except Exception as exception:  # pylint: disable=broad-except
        LOGGER.exception(u'Error exporting CSV %s', courselike_key, exc_info=True)
        if self.status.state != UserTaskStatus.FAILED:
            self.status.fail({'raw_error_msg': text_type(exception)})
        return


def create_export_csv(question_bank, course_key, context, status=None):
    """
    Generates the export csv, or returns None if there was an error.

    Updates the context with any error information if applicable.
    """
    name = question_bank.url_name
    export_file = NamedTemporaryFile(prefix=name + '.', suffix=".csv", mode="r+")
    root_dir = path(mkdtemp())

    try:
        export_questions_to_csv(question_bank, export_file)
        export_file.seek(0)
        if status:
            status.set_state(u'Compressing')
            status.increment_completed_steps()
        LOGGER.debug(u'csv file being generated at %s', export_file.name)
    except SerializationError as exc:
        LOGGER.exception(u'There was an error exporting %s', course_key, exc_info=True)
        parent = None
        try:
