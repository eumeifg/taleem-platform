...
from openedx.custom.question_bank.utils import add_question_tags
    ...
    if usage_key_string:
        ...
        if request.method == 'GET':
            ...
            if 200 <= response.status_code <= 299:
                if xblock.parent and type(xblock.parent) == LibraryUsageLocator:
                    add_question_tags(question=xblock)
                if xblock.category == 'vertical' and \
                    request.json.get('publish') == 'make_public':
                    course_title = CourseOverview.objects.get(
                        id=xblock.location.course_key,
                    ).display_name
                    title = "There has been an update to {{course_title:{course_title}}}".format(
                        course_title=course_title
                    )
                    section = _get_xblock(xblock.parent, request.user)
                    chapter = _get_xblock(section.parent, request.user)
                    if datetime.now(UTC) > xblock.start and datetime.now(UTC) > chapter.start:
                        if published:
                            notification_type = 'lesson_changed'
                            message = "A lesson has been updated to {{course_title:{course_title}}}".format(
                                course_title=course_title
                            )
                        else:
                            notification_type = 'lesson_added'
                            message = "A new lesson has been added to {{course_title:{course_title}}}".format(
                                course_title=course_title
                            )
                        send_announcement_notification.apply_async(kwargs={
                            "title": title,
                            "message": message,
                            ...
                            "data": {
                                ...
                                'sequential_id': str(xblock.parent),
                                'lesson_id': str(xblock.location),
                                'lesson_title': xblock.display_name,
                            }}, countdown=60)
            return response
    ...
    if response_format == 'json' or 'application/json' in request.META.get('HTTP_ACCEPT', 'application/json'):
        with modulestore().bulk_operations(usage_key.course_key):
            if usage_key.block_type == 'vertical':
                subsection = modulestore().get_item(usage_key)
                if len(subsection.get_children()) == 0:
                    _create_discussion_xblock(request, str(subsection.location))
...
def _create_item(request):
    ...
    if isinstance(usage_key, LibraryUsageLocator):
        # Below categories are not allowed to be added in content library.
        if category in ['discussion']:
            return HttpResponseBadRequest(
    ...
    special_categories = {'vertical', 'chapter', 'sequential', 'discussion'}
    if not isinstance(usage_key, LibraryUsageLocator):
        if category not in special_categories:
            discussion_block = _auto_insert_discussion_xblock(request, usage_key, created_block)
        elif category == 'vertical':
            _create_discussion_xblock(request, str(created_block.location))
...
def _auto_insert_discussion_xblock(request, usage_key, created_xblock):
    """
    Insert a new discussion xblock at the end of all xblocks or move the discussion block to the end if already present.
    """
    subsection = modulestore().get_item(usage_key)
    if any(isinstance(t, DiscussionXBlock) for t in subsection.get_children()):
        _move_discussion_xblock_to_the_end(request, usage_key, subsection)
        return created_xblock
    elif len(subsection.get_children()) == 0:
        return _create_discussion_xblock(request, str(usage_key))


def _create_discussion_xblock(request, parent_locator):
    """
    Create discussion xblock.
    """
    from contentstore.views.item import create_xblock_info

    discussion_xblock = create_xblock(
        parent_locator=parent_locator,
        user=request.user,
        category='discussion',
    ...
    return discussion_xblock


def _move_discussion_xblock_to_the_end(request, usage_key, subsection):
    """
    Move the discussion block to the end.
    """
    children = [str(t.location) for t in subsection.get_children() if not isinstance(t, DiscussionXBlock)] + \
               [str(t.location) for t in subsection.get_children() if isinstance(t, DiscussionXBlock)]
    xblock = _get_xblock(usage_key, request.user)
    response = _save_xblock(
        request.user,
        xblock,
...
def _get_release_date(xblock, user=None):
    ...
    try:
        reset_to_default = True if not xblock.start else xblock.start.year < 1900
    except ValueError:
