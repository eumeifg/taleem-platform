...
from contentstore.tests.utils import AjaxEnabledTestClient, CourseTestCase, parse_json, LIBRARY_REST_URL
...
class UnitTestLibraries(CourseTestCase):
    ...
    def test_no_duplicate_libraries(self):
        ...
        self.assertIn('Unable to create library', parse_json(response)['ErrMsg'])
        self.assertEqual(response.status_code, 400)
    ...
    def test_get_lib_info(self):
        ...
        # Whenever a new library is created,
        # It will contains these xBlocks pre-installed in advanced_module_list.
        self.library_setting_url = get_url(lib_key, 'advanced_library_settings_handler')
    ...
    def test_get_component_templates(self):
        """
        Verify that template for adding discussion are not in content libraries
        but we have the advanced components to content libraries.
        """
        lib = LibraryFactory.create()
