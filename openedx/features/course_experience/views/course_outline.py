...
class CourseOutlineFragmentView(EdxFragmentView):
    ...
    def render_to_fragment(self, request, course_id, user_is_enrolled=True, **kwargs):  # pylint: disable=arguments-differ
        ...
        if not request.user.is_authenticated or not CourseEnrollment.is_enrolled(request.user, course_id):
            course_block_tree['children'] = course_block_tree.get('children')[:3]
