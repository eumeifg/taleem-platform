...
from openedx.custom.payment_gateway.utils import get_course_price
...
def _register_course_home_messages(request, course, user_access, course_start_data):
    ...
    if not user_access['is_anonymous'] and not user_access['is_staff'] and \
        ...
        if CourseMode.is_masters_only(course.id):
            ...
            is_eligible_to_enroll = False
            course_price = get_course_price(course.id)
            ...
            if is_eligible_to_enroll:
                ...
                redirect_url = reverse('payment_gateway:vouchers_page', args=(str(course.id), ))
                CourseHomeMessages.register_info_message(
                    request,
                    Text(_(
                        u'{open_buy_now_link}Buy now{close_buy_now_link} to access the full course.'
                    )).format(
                        open_buy_now_link=HTML('<a href="' + redirect_url + '">'),
                        close_buy_now_link=HTML('</a>')
                    ),
                    title=title
                )
        else:
