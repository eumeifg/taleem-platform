...
def learner_profile_context(request, profile_username, user_is_staff):
    ...
    import logging
    logging.info(request.user.profile.phone_number)

    own_profile = (logged_in_user.username == profile_username)

    is_teacher_or_staff = logged_in_user.ta3leem_profile.user_type == UserType.teacher.name or user_is_staff
    set_default_profile_image = not own_profile and not is_teacher_or_staff
    account_settings_data = get_account_settings(
        request,
        [profile_username],
    ...
    context = {
        ...
        'data': {
            ...
            'state': list(logged_in_user.profile.STATE_CHOICES),
            'grade_options': Ta3leemUserProfile.GRADE_CHOICES,
            'category_selection':list(Ta3leemUserProfile.CATEGORIES_CHOICES),
