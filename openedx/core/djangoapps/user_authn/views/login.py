...
from openedx.custom.taleem.utils import (
    ...
    create_and_send_second_password,
    get_attempt_count,
    log_attempt,
    TA3LEEM_CAPTCHA_DISABLE_SWITCH
)
...
def _generate_not_activated_message(user):
    ...
    not_activated_message = Text(_(
        ...
        u'{paragraph_act_email_send_msg_start}We just sent an activation link to {email_strong}.{paragraph_act_email_send_msg_end} If '
        u'you do not receive an email, {link_send_act_email_start} Click Here To Resend {link_send_act_email_end} or check your spam folders or '
        u'{link_start}contact {platform_name} Support{link_end}.'
    )).format(
        platform_name=platform_name,
        blank_lines=HTML('<br/><br/>'),
        paragraph_act_email_send_msg_start=HTML(u'<p id="activation_email_send_message">'),
        paragraph_act_email_send_msg_end=HTML(u'</p>'),
    ...
    logging.info('======= email msg created')
...
def finish_auth(request):  # pylint: disable=unused-argument
    ...
    """
    if not TA3LEEM_CAPTCHA_DISABLE_SWITCH.is_enabled():
...
def login_user(request, skip_login=False):
    """
    AJAX request to log in the user.

    Arguments:
        request (HttpRequest)
        skip_login (bool): A flag to skip login for additional functionality.
...
class LoginSessionView(APIView):
    ...
    def post(self, request):
        ...
        """
        if not TA3LEEM_CAPTCHA_DISABLE_SWITCH.is_enabled():
            if get_attempt_count(request) >= settings.FAILED_LOGGING_THROTTLE_LIMIT:
                return JsonResponse({
                    'success': False,
                    'redirect_url': reverse('taleem:taleem_captcha'),
                    'error_code': 'too-many-attempts',
                    'value': _('This account has been temporarily locked due '
                               'to excessive login failures. Try again later.'),
                }, status=403)
            log_attempt(request)
        ...
        if settings.DISABLE_2FA:
            ...
            response = login_user(request, skip_login=True)

        # Let the default flow handle errors.
        if response.status_code != 200:
            return response
        else:
            request.session['USER_POST_DATA'] = request.POST
            try:
                create_and_send_second_password(request.POST.get('email'))
            except User.DoesNotExist:
                return HttpResponse(
                    'Something not right, please try again. If error persists then contact support.',
                    status=501
                )
