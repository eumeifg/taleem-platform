...
from openedx.core.djangoapps.user_authn.views.secondary_password import get_second_password_form
...
from openedx.custom.taleem.utils import get_attempt_count, TA3LEEM_CAPTCHA_DISABLE_SWITCH
    ...
    if third_party_auth.is_enabled():
        ...
        if running_pipeline:
            ...
            if current_provider:
                ...
                # Ta3leem: Removed enterprise condition in above line
...
def login_and_registration_form(request, initial_mode="login"):
    ...
    if not TA3LEEM_CAPTCHA_DISABLE_SWITCH.is_enabled():
        if get_attempt_count(request) >= settings.FAILED_LOGGING_THROTTLE_LIMIT:
            return redirect(reverse('taleem:taleem_captcha') + '?next={redirect_url}'.format(redirect_url=redirect_url))
    ...
    context = {
        'data': {
            ...
            'second_password_form_desc': json.loads(form_descriptions['second_password']),
            ...
            'disable_2fa': settings.DISABLE_2FA,
        },
...
def _get_form_descriptions(request):
    ...
    return {
        ...
        'second_password': get_second_password_form().to_json(),
