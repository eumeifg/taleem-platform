...
DISABLE_CONFIRM_PASSWORD_MOBILE_APP = WaffleFlag(
    waffle_namespace=WaffleFlagNamespace(name=u'registration'),
    flag_name=u'disable_confirm_password_mobile_app',
    flag_undefined_default=False
)


@transaction.non_atomic_requests
def create_account_with_params(request, params, skip_login=False, skip_activation_email=False):
    """
    ...
    * Duplicate email raises a ValidationError (rather than the expected
    ...
    if skip_email or skip_activation_email:
...
class RegistrationView(APIView):
    ...
    def post(self, request):
        ...
        # for mobile requests set default data
        if is_request_from_mobile_app(request) or data.get("is_request_from_mobile_app"):
        ...
        if DISABLE_CONFIRM_PASSWORD_MOBILE_APP.is_enabled():
            if is_request_from_mobile_app(request) or data.get("is_request_from_mobile_app"):
                data.update({
