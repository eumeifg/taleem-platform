...
class AccessTokenView(_DispatchingView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        # Hack added for translating the "Invalid credential given." message.
        if response.status_code == 400:
            response_content = list(response._container)  # pylint: disable=protected-member
            response.content = b''
            for item in response_content:
                translated_response = {}
                response_item = json.loads(item)
                for item_response in response_item.keys():
                    translated_response[item_response] = _(response_item[item_response])

                response.write(json.dumps(translated_response))
