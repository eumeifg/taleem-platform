...
from openedx.custom.taleem.utils import applied_for_teacher_account, user_is_teacher, user_is_ta3leem_admin
...
def account_settings_context(request):
    ...
    enable_teacher_account_field = True
    if (user.is_staff or user.is_superuser or user_is_ta3leem_admin(user)
        or user_is_teacher(user) or applied_for_teacher_account(user)):
        enable_teacher_account_field = False
    ...
    context = {
        ...
        'fields': {
            'country': {
                ...
                'options': [(choice[0], _(choice[1])) for choice in UserProfile.FIRST_DAY_OF_WEEK_CHOICES],
            }, 'date_format': {
                'options': [(choice[0], _(choice[1])) for choice in UserProfile.DATE_FORMAT_CHOICES],
            }, 'receive_reminder_notifications': {
                'options': [(choice[0], _(choice[1])) for choice in UserProfile.RECEIVE_REMINDER_NOTIFICATIONS_CHOICES]
            }
        },
        ...
        'enable_teacher_account_field': enable_teacher_account_field,
