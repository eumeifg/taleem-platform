...
class UserReadOnlySerializer(serializers.Serializer):
    ...
    def to_representation(self, user):
        ...
        try:
            ...
            user_ta3leem_profile = None
            LOGGER.warning(u"user ta3leem profile for the user [%s] does not exist", user.username)
        ...
        data = {
            ...
            "phone_number": None,
            "grade": 'NA',
            "grade_display": 'Not Available',
            "grade_display_arabic": 'Not Available',
            "grade_display_english": 'Not Available',
            "calendar_first_day_of_week": None,
            "calendar_date_format": None,
            "calendar_receive_reminder_notifications": None,
            "account_type": None,
            }
        ...
        if user_profile:
            ...
            if user_profile.country:
                if is_request_from_mobile_app(self.context.get('request')):
                    country_data = {
                        'code': user_profile.country.code,
                        'name': user_profile.country.name
                    }
                else:
                    country_data = AccountLegacyProfileSerializer.convert_empty_to_None(
            ...
            data.update(
                {
                    ...
                    "profile_image": AccountLegacyProfileSerializer.get_profile_image(
                        ...
                        get_default_female_avatar=self.context.get('set_default_profile_image') and user_profile.gender == 'f'
                    ),
        ...
        if user_ta3leem_profile:
            data['account_type'] = user_ta3leem_profile.get_user_type_display()
            data['phone_number'] = str(user_ta3leem_profile.phone_number or '')
            data['sponsor_mobile_number'] = str(user_ta3leem_profile.sponsor_mobile_number or '')
            data['grade'] = user_ta3leem_profile.grade
            data['grade_display'] = user_ta3leem_profile.get_grade_display()
            data['college'] = str(user_ta3leem_profile.college)
            with translation.override('ar'):
                data['category_selection'] = user_ta3leem_profile.category_selection
            with translation.override('ar'):
                data['grade_display_arabic'] = user_ta3leem_profile.get_grade_display()
            with translation.override('en'):
                data['grade_display_english'] = user_ta3leem_profile.get_grade_display()
        ...
        ta3leem_fields = list(extra_settings.keys()) + [
            'grade', 'grade_display', 'grade_display_arabic', 'account_type', 'grade_display_english']
        fields = fields + list(extra_settings.keys()) + ta3leem_fields
...
class AccountLegacyProfileSerializer(serializers.HyperlinkedModelSerializer, ReadOnlyFieldsSerializerMixin):
    ...
    @staticmethod
    def get_profile_image(user_profile, user, request=None, get_default_female_avatar=False):
        """
        Returns metadata about a user's profile image.
        """
        data = {'has_image': user_profile.has_profile_image}
        urls = get_profile_image_urls_for_user(user, request, get_default_female_avatar)
        data.update({
