...
from openedx.custom.taleem.utils import (
    ...
    update_user_calendar_settings,
    update_grade_if_needed,
    update_phone_number,
    update_sponsor_number,
    update_category_choice
)
...
def get_account_settings(request, usernames=None, configuration=None, view=None, set_default_profile_image=False):
    ...
    Args:
        ...
        set_default_profile_image (boolean): An optional boolean parameter which enforces to add the default female
            avatar.
    ...
    for user in requested_users:
        ...
        serialized_users.append(UserReadOnlySerializer(
            ...
            context={
                'request': request,
                'set_default_profile_image': set_default_profile_image,
            }
        ).data)
...
def update_account_settings(requesting_user, update, username=None):
    ...
    try:
        ...
        _update_college_if_needed(update, ta3leem_profile)
        _update_state_if_needed(update, user_profile)
        update_grade_if_needed(update, user)
        update_user_calendar_settings(update, user)
        update_phone_number(update, user)
        update_sponsor_number(update,user)
        update_category_choice(update,user)
...
def _update_college_if_needed(data, ta3leem_profile):
    if ta3leem_profile and 'college' in data:
        ta3leem_profile.college_id = data['college']
...
def get_phone_number_validation_error(phone_number):
    ...
    """
    return _validate(_validate_phone_number, errors.AccountPhoneNumberInvalid, phone_number)
...
def _validate_phone_number(phone_number):
    ...
    """
    if phone_number == '':
        raise errors.AccountPhoneNumberInvalid(accounts.REQUIRED_FIELD_PHONE_NUMBER_MSG)

    try:
        phone_number = parse(phone_number, None)
    except NumberParseException:
        raise errors.AccountPhoneNumberInvalid(accounts.PHONE_NUMBER_BAD_TYPE_MSG)
    else:
        if not is_valid_number(phone_number):
            raise errors.AccountPhoneNumberInvalid(accounts.INVALID_PHONE_NUMBER)
