...
def _get_default_female_profile_image_urls(name, storage, file_extension=PROFILE_IMAGE_FILE_EXTENSION, version=None):
    """
    Returns a dict containing the urls for a complete set of profile images,
    keyed by "friendly" name (e.g. "full", "large", "medium", "small").
    """
    def _make_url(size):
        url = storage.url(
            '{name}_{size}.{file_extension}'.format(name=name, size=size, file_extension=file_extension)
        )
        # Return the URL, with the "v" parameter added as its query
        # string with "?v=". If the original URL already includes a
        # query string (such as signed S3 URLs), append to the query
...
def get_profile_image_urls_for_user(user, request=None, get_default_female_avatar=False):
    """
    ...
    """
    try:
        if get_default_female_avatar:
            urls = _get_default_female_profile_image_urls(
                'images/profiles/female_avatar',
                staticfiles_storage,
                file_extension=settings.PROFILE_IMAGE_DEFAULT_FILE_EXTENSION,
            )
        elif user.profile.has_profile_image:
