...
from openedx.custom.taleem.form_fields import RadioChoiceField
...
class FormDescription(object):
    ...
    ALLOWED_TYPES = [
        "text", "email", "select", "textarea", "checkbox",
        "radio", "plaintext", "password", "hidden", "tel",
    ]
    ...
    def add_field(
        ...
        if field_type in ("select", "radio", ):
            if options is not None:
                ...
                if name in ('school', 'organization',):
                    existing_default_value = get_default_organization(name)
                else:
                    existing_default_value = field_override.get('defaultValue', default)
