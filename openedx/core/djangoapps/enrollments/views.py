...
from openedx.custom.timed_exam.enrollment import bulk_enroll
...
class UnenrollmentView(APIView):
    """
        **Use Cases**
            ...
            * Unenroll a single user from all courses or given single course.
        ...
        **Example Requests**
            ...
            POST /api/enrollment/v1/enrollment {
                ...
                "course_id": "course-v1:taleem+test_11+1122"
            }
        ...
        **POST Parameters**
            ...
            * course_id (optional): The id of the course from which the user being unenrolled.
    ...
    def post(self, request):
        ...
        try:
            ...
            if username != request.user.username:
                allowed_to_perform = any([
                    request.user.ta3leem_profile.user_type == UserType.teacher.name,
                    request.user.is_staff,
                    request.user.is_superuser,
                ])
                if not allowed_to_perform:
                    return Response(u'You are not allowed to perform this action.', status=status.HTTP_403_FORBIDDEN)
            # Get the course id if given
            ...
            if course_id:
                ...
                return Response(api.unenroll_user_from_given_course(username, course_id))
...
class EnrollmentListView(APIView, ApiKeyPermissionMixIn):
    ...
    def post(self, request):
        ...
        if mode not in (CourseMode.AUDIT, CourseMode.HONOR, CourseMode.TIMED, None) and not has_api_key_permissions \
                and not GlobalStaff().has_user(request.user):
            return Response(
...
class BulkEnrollmentView(APIView):
    """
        **Use Cases**
            ...
            * Bulk enroll users from the given CSV file.

        **Example Requests**

            POST /api/enrollment/v1/bulk/enrollment {
                "csv": "csv file"
        ...
        **POST Parameters**
            ...
            * csv: CSV file containing emails.

            * course_id: The id of the course to which the users being enrolled.

            * email (optional): If a single user needs to be enrolled.
    ...
    def post(self, request):
        """
        Enrolls the specified user to the given course.
        """
        allowed_to_perform = any([
            request.user.ta3leem_profile.user_type == UserType.teacher.name,
            request.user.is_staff,
        ...
        try:
            ...
            enrolled = bulk_enroll(csv_file, post_data)
        except (InvalidCSVDataError, ValidationError, InvalidEmailError) as error:
            return Response(
        ...
        return Response(
            data={
                'url': reverse(
                    'timed_exam:enrollment_dashboard',
                    args=(post_data.get('course_key_string'),)
                ),
                'enrolled': enrolled,
            },
