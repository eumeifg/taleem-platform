...
def create_course_enrollment(username, course_id, mode, is_active, check_access=True):
    ...
    Args:
        ...
        check_access (boolean): (Optional) Enforce access check or not.
...
def unenroll_user_from_given_course(username, course_key):
    """
    Set given of a user's enrollments to inactive.
    :param username: The user being unenrolled.
    :param course_key: Course id from which user is being unenrolled.
    :return: A course from which the user was unenrolled.
    """
    user = _get_user(username)
    if user:
