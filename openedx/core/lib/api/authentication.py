...
class Ta3leemBearerAuthentication(BearerAuthentication):
    """
    Token to be sent in request param `ta3_token`
    """
    ...
    def authenticate(self, request):
        ...
        set_custom_metric("BearerAuthentication", "Failed")  # default value
        access_token = request.GET.get('ta3_token')

        if not access_token:
            raise AuthenticationFailed({
                'error_code': OAUTH2_TOKEN_ERROR_NOT_PROVIDED,
                'developer_message': 'Invalid token param. No credentials provided.'})
